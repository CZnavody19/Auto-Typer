...
def capture(repeat, reverse, begins, contain, nospace):
	...
	text = typer.capture(repeat, reverse, contain, begins, nospace, parsed_options["language"])
...
def open_settings():
	def on_closing():
		with open("options.json", "w") as options:
			options.write(json.dumps({"blocked_keys" : blocked_keys.get("1.0",tk.END)[:-1], "reset_options": resetoptions_value.get(), "language" : language_value.get()}))
	...
	settings_third_line = tk.Frame(settings_window)
	...
	resetoptions_label = tk.Label(settings_third_line, text="Text language: ")
	resetoptions_label.pack(side=tk.LEFT)

	language_value.set(parsed_options["language"])
	language_picker = tk.OptionMenu(settings_third_line, language_value, *languages)
	language_picker.pack()


	settings_first_line.pack()
	settings_second_line.pack()
	settings_third_line.pack()
